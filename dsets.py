...
MaskTuple = namedtuple('MaskTuple',
                       ...
                       'raw_candidate_mask, candidate_mask, lung_mask, '
                       'neg_mask, pos_mask')

CandidateInfoTuple = namedtuple('CandidateInfoTuple',
                                'isNodule_bool, hasAnnotation_bool, '
                                'isMal_bool, diameter_mm, series_uid, '
                                'center_xyz')
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob('C:/data/subset*/*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open('C:/data/annotations_with_malignancy.csv', "r") as f:
    ...
    with open('C:/data/candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
    ...
    def getRawCandidate(self, center_xyz, width_irc):
        ...
        for axis, center_val in enumerate(center_irc):
            ...
            assert center_val >= 0 and center_val < self.hu_a.shape[axis], repr(
                [self.series_uid, center_xyz, self.origin_xyz, self.vxSize_xyz,
                 center_irc, axis])
...
class Luna2dSegmentationDataset(Dataset):
    def __init__(self,
        ...
        log.info("{!r}: {} {} series, {} slices, {} nodules".format(
            ...
            {None: 'general', True: 'validation', False: 'training'}[
                isValSet_bool],
...
class PrepcacheLunaDataset(Dataset):
    def __init__(self, *args, **kwargs):
        ...
        self.candidateInfo_list = getCandidateInfoList()
        self.pos_list = [nt for nt in self.candidateInfo_list if
                         nt.isNodule_bool]
...
class TvTrainingLuna2dSegmentationDataset(torch.utils.data.Dataset):
    def __init__(self, isValSet_bool=False, val_stride=10,
                 contextSlices_count=3):
    ...
    def __getitem__(self, i):
        ...
        return self.imgs[i, :, oh: oh + 64, ow: ow + 64], 1, self.masks[i,
                                                             sl: sl + 1,
                                                             oh: oh + 64,
                                                             ow: ow + 64].to(
